...
from trainer_sts_gen import Trainer
from data_collector import T2TDataCollator
from utils import freeze_embeds, assert_not_all_frozen

from my_modeling_mt5_origin import MyMT5ForConditionalGeneration
...
class ModelArguments:
    ...
    ### 2022/12/19
    '''
    multiple_negative_ranking_loss: Optional[bool] = field(
        default = True,
        metadata={"help": "Set to [True] if choose to learn the cos_sim from the positive pair of context and answer" }
    )
    '''
...
class DataTrainingArguments:
    ...
    lambda_mt5: Optional[float] = field(
        default=0.25,
        metadata={"help": "Weight of Cross Entropy Loss of the mT5"},
    )
    lambda_mnrl: Optional[float] = field(
        default=0.75,
        metadata={"help": "Weight of Cross Entropy Loss of the MNRL"},
    )
...
def main(args_file=None):
    ...
    model = MyMT5ForConditionalGeneration.from_pretrained(
    ...
    trainer = Trainer(
        ...
        lambda_mt5 = data_args.lambda_mt5,
        lambda_mnrl = data_args.lambda_mnrl
    )
